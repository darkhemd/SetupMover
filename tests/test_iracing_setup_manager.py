...
def test_detect_format_unknown():
    assert detect_format("too_short") == (None, None, None)


def test_stem_format_label():
    assert stem_format_label("a_b_c_d_e") == "A"
    assert stem_format_label("VRS_26S2_RS3Gen2_Spa_Q") == "C"
    assert stem_format_label("a_b_c_d_e_f") == "B"
...
def test_build_dest_path(mode: str, expected_suffix: Path):
    ...
    got = build_dest_path(dest, "bmwm4gt3", "Spa", mode, "26S2")
...
def test_plan_sto_operations_ready(tmp_path: Path):
    ...
    plan = plan_sto_operations(dest, [(f.name, f.resolve())], aliases, "none", "26S2")
    assert len(plan) == 1
    assert plan[0].ready
...
def test_plan_sto_operations_no_alias(tmp_path: Path):
    ...
    f.write_text("x", encoding="utf-8")
    plan = plan_sto_operations(dest, [(f.name, f.resolve())], {}, "none", "26S2")
    assert len(plan) == 1
...
def test_plan_sto_operations_duplicate_basename(tmp_path: Path):
    ...
    aliases = {"RS3Gen2": "audirs3lmsgen2"}
    plan = plan_sto_operations(dest, sources, aliases, "none", "26S2")
    assert len(plan) == 2
    assert len([e for e in plan if e.status == "duplicate_name"]) == 1
    assert sum(1 for e in plan if e.ready) == 1


def test_merge_path_history():
    h: list[str] = []
    merge_path_history(h, "/a")
    merge_path_history(h, "/b")
    merge_path_history(h, "/a")
    assert h == ["/a", "/b"]


def test_dest_looks_like_setups_root(tmp_path: Path):
    assert dest_looks_like_setups_root(tmp_path) is False
    good = tmp_path / "setups_like"
    good.mkdir()
    (good / "bmwm4gt3").mkdir()
    assert dest_looks_like_setups_root(good) is True


def test_pick_rename_destination(tmp_path: Path):
    f = tmp_path / "x.sto"
    f.write_text("1", encoding="utf-8")
    alt = pick_rename_destination(f)
...
def test_collect_sto_sources_flat(tmp_path: Path):
    ...
    out = collect_sto_sources(src, False)
    assert len(out) == 1
...
def test_collect_sto_sources_recursive(tmp_path: Path):
    ...
    out = collect_sto_sources(src, True)
    assert len(out) == 1
...
def test_load_aliases_json_file(tmp_path: Path):
    p = tmp_path / "a.json"
    p.write_text('[{"alias": "X", "folder": "bmwm4gt3", "note": ""}]', encoding="utf-8")
    d = load_aliases_json_file(p)
    assert d == {"X": "bmwm4gt3"}
